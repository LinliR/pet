<template>
  <el-row style="height: 100%;">
    ...
    <el-col :span="4" style="height: 100%;">
      <el-card style="height: 100%;" shadow="none">
        <el-col :span="24">
          ...
          >
            <el-menu-item :index="index" v-for="(message,index) in userList" @click="loadUserMsg(message)">
              <el-avatar :size="40"
                         src="https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png"/>
              <span style="margin-left: 10px">{{
                  message.recipientsName == currentUser.userName ? "" : message.recipientsName
                }}</span>
            </el-menu-item>
    ...
    <el-col :span="20" style="height: 100%;background: rgba(0,0,0,0.1);">
      <div id="messageContent" ref="dropDownRef" style="height: 80%;display: block;overflow-y: scroll">
        <el-card>
          <!--左边头像-->
          <el-row v-for="(item,index) in messageList" :key="index" style="margin-top: 20px">
            ...
            <el-col :span="18">
              <div style="display: inline-block;padding: 5px;border-radius: 20px;margin: 0 10px"
...
export default {
  ...
  methods: {
    ...
    fromUserPet() {
      ...
      let dialogueUserId = localStorage.getItem("dialogueUserId");

      if (!dialogueUserId) {
        return;
      }
      this.recipientsId = dialogueUserId;
      let url = "/user/get/" + dialogueUserId;
      httpUtil.get(url).then(res => {
        console.log(res);
        let recipient = {
          recipientsId: dialogueUserId,
          recipientsName: res.data.userName,
          addresserId: localStorage.getItem("userId")
        }
        let userList = this.userList;
        userList.unshift(recipient);
        this.userList = userList;
        console.log(this.userList)
      })
      localStorage.removeItem("dialogueUserId")
